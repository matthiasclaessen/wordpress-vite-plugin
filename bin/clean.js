...
const main = () => {
  const manifestPaths = argument(`manifest`) ? [argument(`manifest`)] : option(`ssr`) ? [`./bootstrap/ssr/ssr-manifest.json`, `./bootstrap/ssr/manifest.json`] : [`./public/build/manifest.json`];
  ...
  if (orphanedAssets.length === 0) {
    ...
    orphanedAssets.forEach((asset) => {
      ...
      option('dry-run') ? info(`Orphaned assets [${path}] would be removed.`) : info(`Removing orphaned assets [${path}].`);
