...
export default function wordpress(config: string | string[] | PluginConfig): [WordPressPlugin, ...Plugin[]] {
  ...
  return [resolveWordPressPlugin(pluginConfig), ...(resolveFullReloadConfig(pluginConfig) as Plugin[])];
}
...
function resolveDevelopmentEnvironmentServerConfig(host: string | boolean | null):
  ...
  if (typeof configPath === 'undefined') {
    throw Error(`Unable to find the Herd or Valet configuration directory. Please check if they are correctly installed.`);
  }
